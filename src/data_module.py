...
def train_test_validation_split(features, targets):
    ...
    X_train, X_validation, y_train, y_validation = train_test_split(features, targets, test_size=VALIDATION_SPLIT, random_state=RANDOM_SEED)

    return X_train, y_train, X_test, y_test, X_validation, y_validation

def quartiles_dummy_variables(features):
    
    X_quartiles = []

    #Separete each colum in 4 quartiles
    for col in features.columns:
        bins = pd.qcut(features[col], q=4, duplicates='drop')
